...
class ImageDatasetRandom(Dataset):
    """wrap up the random dataset into our own"""
    def __init__(self, pt_dataset, d_img, vocab_size):
        self.pt_dataset = pt_dataset
        self.clusters = None  # dummy
        self.d_img = d_img
...
# def make_dictionary(train_data, dict_size, d_img):
#     # get random 32 pixels per image and stack them all up as rgb values to get half a million random pixels
...
#     idx = np.arange(0, 862403, 3)  # say 1fps
#     for i in idx:
#         x, _ = train_data[i]
...
def generate_samples(model, train_dataset, n_samples, filename):
    ...
    pixels = sample(model, start_pixel, train_dataset.d_img * train_dataset.d_img - 1, temperature=0.96, sample=True, top_k=128)
    ...
    for i in range(n_samples):
        ...
        img = cluster_centers[pxi].view(train_dataset.d_img, train_dataset.d_img, 3)
    ...
    print(all_imgs.size())
    save_image(all_imgs, filename, nrow=n_samples//4, padding=1, normalize=True)

def generate_from_half(x, model, train_dataset, filename):
    """
    Generate the lower half given the upper half.
    """
    cluster_centers = train_dataset.clusters
    # TODO: better handle size here
    pixels_0 = sample(model, x[:, :4092].cuda(), 4, temperature=0.95, sample=True, top_k=128)
    print('sample 0')
    pixels_1 = sample(model, x[:, :2048].cuda(), 2048, temperature=0.95, sample=True, top_k=128)
    print('sample 1')
    pixels_2 = sample(model, x[:, :2048].cuda(), 2048, temperature=0.95, sample=True, top_k=128)
    print('sample 2')
    ...
    all_imgs = torch.cat(all_imgs, 0)
    print(all_imgs.size())
    save_image(all_imgs, filename, nrow=16, padding=1, normalize=True)
